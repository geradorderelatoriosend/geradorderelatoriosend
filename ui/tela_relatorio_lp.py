...
class RelatorioLPDialog(QDialog):
    """
    Tela para gerar Relatório de Líquido Penetrante (LP).
    Herdando dados do cliente (EMPRESA, ENDEREÇO, etc.), igual ao US.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Novo Relatório de Líquido Penetrante (LP) - RL Metais")
        self.resize(650, 600)
        ...
        # Produtos penetrante / revelador / condições
        ...
        form.addRow("Penetrante - Data de Fabricação", self.ed_fab_pen)
        form.addRow("Penetrante - Validade", self.ed_val_pen)
        form.addRow("Penetrante - Lote", self.ed_lote_pen)

        form.addRow("Revelador - Data de Fabricação", self.ed_fab_rev)
        form.addRow("Revelador - Validade", self.ed_val_rev)
        form.addRow("Revelador - Lote", self.ed_lote_rev)
    ...
    def _on_ok(self):
        ...
        session = get_session()
        try:
            # Garante que o tipo de relatório LP exista
            tipo = (
                session.query(TipoRelatorio)
                .filter_by(nome="Líquido Penetrante - LP")
                .first()
            )
            if not tipo:
                from models import TipoRelatorio as TipoRelatorioModel
                tipo = TipoRelatorioModel(
                    nome="Líquido Penetrante - LP",
                    descricao="Relatório de Líquido Penetrante",
                    schema_json="{}",   # ← PREENCHE OBRIGATÓRIO
                    template_path="templates/LP_TEMPLATE.docx",
                )
                session.add(tipo)
                session.commit()
            ...
            caminho_arquivo = generate_lp_report(merged_dados, template_path, output_dir)

            entrada = EntradaRelatorio(
                cliente_id=cliente.id,
                tipo_relatorio_id=tipo.id,
                relatorio_num=merged_dados.get("NUMRELATORIO"),
                titulo_personalizado=f"Relatório {merged_dados.get('NUMRELATORIO')}-LP",
            ...
            QMessageBox.information(
                self,
                "Sucesso",
                f"Relatório de LP gerado em:\n{caminho_arquivo}",
            )
            # Mantém a janela aberta para gerar mais relatórios
        except Exception as e:
            session.rollback()
            QMessageBox.critical(self, "Erro", f"Falha ao gerar relatório de LP:\n{e}")
        finally:
            session.close()
